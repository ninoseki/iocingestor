...
from iocingestor.artifacts import STRING_MAP
from iocingestor.exceptions import IngestorError, PluginError

SOURCE = "iocingestor.sources"
OPERATOR = "iocingestor.operators"
...
class Config:
    ...
    @staticmethod
    def _load_plugin(plugin_type: str, plugin: str):
        """Returns plugin class or raises an exception.

        :raises: iocingestor.exceptions.PluginError
        """
        try:
    ...
    def credentials(self, credential_name: str):
                        ...
                        for artifact in value:
                            try:
                                artifact_types.append(
                                    STRING_MAP[artifact.lower().strip()]
                                )
                            except KeyError:
