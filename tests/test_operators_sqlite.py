...
class TestThreatSQLite(unittest.TestCase):
    ...
    def test_artifact_types_are_set_if_passed_in_else_default(self):
        ...
        self.assertEqual(
            iocingestor.operators.sqlite.Plugin(
                ":memory:", artifact_types=artifact_types
            ).artifact_types,
            artifact_types,
        )
        self.assertEqual(
            iocingestor.operators.sqlite.Plugin(":memory:").artifact_types,
            [
                iocingestor.artifacts.Domain,
                iocingestor.artifacts.Hash,
                iocingestor.artifacts.IPAddress,
                iocingestor.artifacts.URL,
                iocingestor.artifacts.Task,
            ],
        )
    ...
    def test_filter_string_and_allowed_sources_are_set_if_passed_in(self):
        self.assertEqual(
            iocingestor.operators.sqlite.Plugin(
                ":memory:", filter_string="test"
            ).filter_string,
            "test",
        )
        self.assertEqual(
            iocingestor.operators.sqlite.Plugin(
                ":memory:", allowed_sources=["test-one"]
            ).allowed_sources,
            ["test-one"],
        )
