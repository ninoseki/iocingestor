...
import iocingestor.sources.github
...
class TestGitHub(unittest.TestCase):
    def setUp(self):
        self.github = iocingestor.sources.github.Plugin("mygithub", "CVE-2018-")

    @patch("iocingestor.sources.github.datetime")
    @responses.activate
    def test_run_returns_saved_state_tasks(self, mock_datetime):
        responses.add(
            responses.GET,
            iocingestor.sources.github.SEARCH_URL,
            body=API_RESPONSE_DATA,
        )
        mock_datetime.datetime.utcnow.return_value = datetime.datetime(
            2018, 4, 30, 17, 5, 13, 194840
        )
        ...
        self.assertIn(
            "Manual Task: GitHub dtrupenn/Tetris", [str(x) for x in artifact_list]
        )
    ...
    def test_requests_until_rel_is_not_next(self):
        ...
        responses.add(
            ...
            body=API_RESPONSE_DATA,
            headers={
                "Link": '<https://api.github.com/search/repositories?q=Test&page=3>; rel="next"'
            },
        )
        responses.add(
            responses.GET,
            ...
            body=API_RESPONSE_DATA,
            headers={
                "Link": '<https://api.github.com/search/repositories?q=Test&page=3>; rel="last"'
            },
        )
