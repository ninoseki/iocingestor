...
import iocingestor.sources.rss
...
class TestRSS(unittest.TestCase):
    ...
    def setUp(self):
        self.rss = iocingestor.sources.rss.Plugin(
            "myrss", "http://rss.mock/rss.xml", "messy"
        )

    @httpretty.activate
    def test_run_respects_saved_state(self):
        httpretty.register_uri(
            httpretty.GET, "http://rss.mock/rss.xml", body=self.RSS_CONTENT
        )
    ...
    def test_run_respects_feed_type(self):
        ...
        messy = iocingestor.sources.rss.Plugin(
            "myrss", "http://rss.mock/rss.xml", "messy"
        )
        clean = iocingestor.sources.rss.Plugin(
            "testrss", "http://rss.mock/rss.xml", "clean"
        )
        afterioc = iocingestor.sources.rss.Plugin(
            "rsss", "http://rss.mock/rss.xml", "afterioc"
        )
    ...
    def test_run_supports_both_link_url(self):
        ...
        self.assertIn("http://example.com/bad/url/10", [str(x) for x in artifacts])
        self.assertIn(
            "https://www.rss.mock/some/url", [a.reference_link for a in artifacts]
        )

        afterioc = iocingestor.sources.rss.Plugin(
            "test", "http://rss.mock/rss.xml", "afterioc"
        )
        saved_state, artifacts = afterioc.run(None)
