...
import iocingestor.sources.twitter


class TestTwitter(unittest.TestCase):
    @patch("twitter.Twitter")
    def setUp(self, Twitter):
        self.twitter = iocingestor.sources.twitter.Plugin("a", "b", "c", "d", "e")
    ...
    def test_init_detects_search_type(self, Twitter):
        # default to @mentions
        twitter = iocingestor.sources.twitter.Plugin("a", "b", "c", "d", "e")
        ...
        twitter = iocingestor.sources.twitter.Plugin(
            "a", "b", "c", "d", "e", slug="test", owner_screen_name="test"
        )
        ...
        self.assertEqual(twitter.endpoint._mock_name, "mentions_timeline")
        twitter = iocingestor.sources.twitter.Plugin(
            "a", "b", "c", "d", "e", owner_screen_name="test"
        )
        self.assertEqual(twitter.endpoint._mock_name, "mentions_timeline")

        # screen_name or user_id => user
        twitter = iocingestor.sources.twitter.Plugin(
            "a", "b", "c", "d", "e", screen_name="test"
        )
        self.assertEqual(twitter.endpoint._mock_name, "user_timeline")
        twitter = iocingestor.sources.twitter.Plugin("a", "b", "c", "d", "e", user_id=1)
        self.assertEqual(twitter.endpoint._mock_name, "user_timeline")

        # q => search
        twitter = iocingestor.sources.twitter.Plugin("a", "b", "c", "d", "e", q="test")
    ...
    def test_run_expands_tco_links_if_available(self):
        ...
        # an obfuscated url and added to the iocingestor.artifacts list
        saved_state, artifact_list = self.twitter.run(None)
        self.assertEqual(len(artifact_list), 3)
        self.assertIn("someurl.com", [str(x) for x in artifact_list])
        self.assertIn("http://someurl.com/test", [str(x) for x in artifact_list])
    ...
    def test_run_includes_nonobfuscated_iff_configured(self, Twitter):
        ...
        new_twitter = iocingestor.sources.twitter.Plugin(
            "a", "b", "c", "d", "e", defanged_only=False
        )
